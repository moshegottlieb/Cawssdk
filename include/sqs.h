...
typedef int (*SQSMessageReceivedCallback)(void* PIM_NONNULL ctx,AWSErrorPolicy policy,const char* PIM_NULLABLE id,const char* PIM_NULLABLE message,const char* PIM_NULLABLE receipt);
void SQSReceiveMessages(AWSObjectRef PIM_NONNULL object, const char* PIM_NONNULL queueUrl,int maxBatch,int timeoutSeconds,void* PIM_NONNULL context,SQSMessageReceivedCallback PIM_NONNULL handler,AWSResult* PIM_NONNULL result);
